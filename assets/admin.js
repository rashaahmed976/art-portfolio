...
const DEFAULT_CONTENT = {
  ...
  // إعدادات الموقع العامة (الهيدر والفوتر)
  "_site_settings": {
...
// ============================================
// إدارة إعدادات الموقع (الهيدر/الفوتر)
// ============================================

function getSiteSettings() {
  const allContent = loadContent();
  return allContent._site_settings || DEFAULT_CONTENT._site_settings;
}

function updateSiteSettings(updates) {
  const allContent = loadContent();
  allContent._site_settings = { ...allContent._site_settings, ...updates };
  saveContent(allContent);
  return allContent._site_settings;
}

function renderSiteSettings() {
  const settings = getSiteSettings();
  const isHomePage = !document.body.getAttribute('data-page-id');
  
  // الهيدر - العنوان الرئيسي
  const headerTitleEl = document.querySelector('[data-site="header-title"]');
  if (headerTitleEl) headerTitleEl.textContent = settings.headerTitle;
  
  // الهيدر - النص الفرعي
  const headerSubtitleEl = document.querySelector('[data-site="header-subtitle"]');
  if (headerSubtitleEl) {
    headerSubtitleEl.textContent = isHomePage 
      ? settings.headerSubtitle 
      : settings.headerSubtitlePages;
  }
  
  // قسم Hero (الصفحة الرئيسية فقط)
  const heroTitleEl = document.querySelector('[data-site="hero-title"]');
  if (heroTitleEl) heroTitleEl.textContent = settings.heroTitle;
  
  const heroSubtitleEl = document.querySelector('[data-site="hero-subtitle"]');
  if (heroSubtitleEl) heroSubtitleEl.textContent = settings.heroSubtitle;
  
  const heroExtraEl = document.querySelector('[data-site="hero-extra"]');
  if (heroExtraEl) heroExtraEl.textContent = settings.heroExtra;
  
  // الفوتر
  const footerTitleEl = document.querySelector('[data-site="footer-title"]');
  if (footerTitleEl) footerTitleEl.innerHTML = `<strong>${escapeHTML(settings.footerTitle)}</strong>`;
  
  const footerSubtitleEl = document.querySelector('[data-site="footer-subtitle"]');
  if (footerSubtitleEl) footerSubtitleEl.textContent = settings.footerSubtitle;
  
  const footerYearEl = document.querySelector('[data-site="footer-year"]');
  if (footerYearEl) footerYearEl.textContent = settings.footerYear;
}
...
function renderPageContent(pageId) {
  // عرض إعدادات الموقع (هيدر/فوتر) في كل الصفحات
  renderSiteSettings();
...
function showHomeAdmin() {
  ...
  panel.innerHTML = `
    ...
    <div class="admin-info" style="margin: 0 0 1.5rem;">
      <strong>👋 مرحباً بكِ في لوحة التحكم</strong>
      <p>عدّلي بيانات الموقع الأساسية (الرأس والذيل) من هنا، أو ادخلي أي صفحة كفاءة لتعديل محتواها.</p>
    </div>
    
    <div style="background: #F7F1E3; padding: 1rem; border-radius: 10px; margin-bottom: 1rem;">
      <h4 style="color: #2C5F2D; margin-bottom: 0.8rem; font-size: 1.1rem;">🎩 رأس الصفحة (Header)</h4>
      
      <div class="admin-section">
        <label>العنوان الرئيسي للموقع</label>
        <input type="text" id="site-header-title" value="${escapeHTML(settings.headerTitle)}">
      </div>
      
      <div class="admin-section">
        <label>النص الفرعي - الصفحة الرئيسية</label>
        <input type="text" id="site-header-subtitle" value="${escapeHTML(settings.headerSubtitle)}">
      </div>
      
      <div class="admin-section">
        <label>النص الفرعي - الصفحات الداخلية</label>
        <input type="text" id="site-header-subtitle-pages" value="${escapeHTML(settings.headerSubtitlePages)}">
      </div>
    </div>
    
    <div style="background: #F7F1E3; padding: 1rem; border-radius: 10px; margin-bottom: 1rem;">
      <h4 style="color: #2C5F2D; margin-bottom: 0.8rem; font-size: 1.1rem;">🎨 قسم الترحيب (Hero)</h4>
      
      <div class="admin-section">
        <label>العنوان الرئيسي للترحيب</label>
        <input type="text" id="site-hero-title" value="${escapeHTML(settings.heroTitle)}">
      </div>
      
      <div class="admin-section">
        <label>النص الترحيبي</label>
        <input type="text" id="site-hero-subtitle" value="${escapeHTML(settings.heroSubtitle)}">
      </div>
      
      <div class="admin-section">
        <label>النص الإضافي تحت الترحيب</label>
        <input type="text" id="site-hero-extra" value="${escapeHTML(settings.heroExtra)}">
      </div>
    </div>
    
    <div style="background: #F7F1E3; padding: 1rem; border-radius: 10px; margin-bottom: 1rem;">
      <h4 style="color: #2C5F2D; margin-bottom: 0.8rem; font-size: 1.1rem;">📄 ذيل الصفحة (Footer)</h4>
      
      <div class="admin-section">
        <label>العنوان الرئيسي للفوتر</label>
        <input type="text" id="site-footer-title" value="${escapeHTML(settings.footerTitle)}">
      </div>
      
      <div class="admin-section">
        <label>النص الفرعي للفوتر</label>
        <input type="text" id="site-footer-subtitle" value="${escapeHTML(settings.footerSubtitle)}">
      </div>
      
      <div class="admin-section">
        <label>السنة الدراسية</label>
        <input type="text" id="site-footer-year" value="${escapeHTML(settings.footerYear)}">
      </div>
    </div>
    
    <div class="admin-actions">
      <button onclick="saveSiteSettings()" class="btn-primary">💾 حفظ إعدادات الموقع</button>
    ...
    <div class="admin-section" style="margin-top: 1.5rem;">
      <label>💡 طريقة تعديل محتوى الكفاءات:</label>
      <p style="font-size: 14px; line-height: 1.8; color: #555;">
        ...
        3. عدّلي المحتوى واحفظي<br>
        4. في النهاية، اضغطي "تصدير" وارفعي ملف <code>content.json</code> على GitHub
      </p>
    </div>
  `;
  document.body.appendChild(panel);
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (pageId) {
    ...
    // الصفحة الرئيسية - عرض إعدادات الموقع فقط
    renderSiteSettings();
  }
