...
// ── Supabase config ──
// After first setup, your URL and key are saved in localStorage.
// For visitors on other devices, we embed them here automatically
// when you save settings (see saveSupabaseConfig below).
let SUPABASE_URL = "";
...
function initSupabase() {
  ...
  if (HARDCODED_URL && HARDCODED_KEY) {
    SUPABASE_URL = HARDCODED_URL;
    SUPABASE_ANON_KEY = HARDCODED_KEY;
    supabaseConfigured = true;
    return;
  }
  // 2. Fall back to localStorage (works on your own device)
...
window.testAndSaveSupabase = async function() {
  ...
  try {
    ...
    status.innerHTML = `
      ...
      <div style="background:#EEF7F0;border:1.5px solid #4A7C59;border-radius:8px;padding:10px;font-size:12px;line-height:1.8">
        <strong>⚠️ خطوة مهمة لمزامنة الزوار:</strong><br>
        افتحي ملف <code>assets/admin.js</code> على GitHub<br>
        وابحثي عن هذين السطرين:<br>
        <code>const HARDCODED_URL = "";</code><br>
        <code>const HARDCODED_KEY = "";</code><br><br>
        وعدّليهم لـ:<br>
        <code>const HARDCODED_URL = "${url}";</code><br>
        <code>const HARDCODED_KEY = "${key}";</code><br><br>
        ثم احفظي (Commit) — بعدها <strong>كل الزوار</strong> سيرون تعديلاتك تلقائياً ✨
      </div>`;
  } catch (e) {
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Show default content immediately so page doesn't look blank
  if (pageId) renderPageContent(pageId);
  else renderSiteSettings();

  // Then load from Supabase (works for ALL visitors, any device)
  if (supabaseConfigured) {
  ...
  // Fallback: try content.json on GitHub (for visitors before Supabase setup)
  try {
